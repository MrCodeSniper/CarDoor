...
public class CustomGridView extends LinearLayout {
	...
	public void setChildClickListener(
			CustomChildClickListener childClickListener) {
	 ...
	 * <p>
	 * 时间: 2014年11月15日 上午10:45:56
	 * <p>
	 */
	private void initView(Context context) {
		this.mContext = context;
		verticalViewWidth = CommUtil.dip2px(mContext, 1);
		View root = View.inflate(mContext, R.layout.gridview_child_layoutview,
				null);
		TextView textView = (TextView) root
				.findViewById(R.id.gridview_child_name_tv);
		...
		viewWidth = (mContext.getResources().getDisplayMetrics().widthPixels - CommUtil
				.dip2px(mContext, 2)) / CustomGroup.COLUMNUM;
	}
	 ...
	 * 作者: 梅雄新 meixx@500wan.com
	 * <p>
	 * 时间: 2014年11月15日 上午10:46:19
	 * <p>
	 */
	public void notifyDataSetChange(boolean needAnim) {
		removeAllViews();
		rowNum = mPlayList.size() / CustomGroup.COLUMNUM
				+ (mPlayList.size() % CustomGroup.COLUMNUM > 0 ? 1 : 0);
		...
		for (int rowIndex = 0; rowIndex < rowNum; rowIndex++) {
			...
			for (int columnIndex = 0; columnIndex < CustomGroup.COLUMNUM; columnIndex++) {
				...
				View root = View.inflate(mContext,
						R.layout.gridview_child_layoutview, null);
				TextView textView = (TextView) root
						.findViewById(R.id.gridview_child_name_tv);
				if (isValidateView) {
					final DargChildInfo tempChilid = mPlayList
							.get(itemInfoIndex);
					//扩展点击事件
			...
			if (needAnim) {
				createHeightAnimator(mParentView,
						CustomGridView.this.getHeight(), rowNum * viewHeight);
			}
		}
	}
	 ...
	 * 描述: TODO
	 * <p>
	 * 参数: @param view 参数: @param start 参数: @param end
	 * <p>
	 * 返回: void
	 * <p>
	 * 异常
