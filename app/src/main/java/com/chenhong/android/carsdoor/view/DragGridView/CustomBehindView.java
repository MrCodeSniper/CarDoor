...
public class CustomBehindView extends GridView {
	...
	private void createFirstDragImage() {
		removeDragImage();
		isDrag = true;
		ImageView ivDelet = (ImageView) mStartDragItemView
				.findViewById(R.id.delet_iv);
		LinearLayout llContainer = (LinearLayout) mStartDragItemView
				.findViewById(R.id.edit_ll);
		if (ivDelet != null) {
			ivDelet.setVisibility(View.VISIBLE);
		}
		if (llContainer != null) {
			llContainer.setBackgroundColor(mContext.getResources().getColor(
					R.color.item_bg));
		}
		mStartDragItemView.setDrawingCacheEnabled(true);
		...
		mDragImageView.setBackgroundColor(mContext.getResources().getColor(
				R.color.item_bg));//设置背景颜色
	 ...
	 * 描述: TODO
	 * <p>
	 * 参数: @param bitmap 参数: @param downX 按下的点相对父控件的X坐标 参数: @param downY
	 * 按下的点相对父控件的Y坐标
	 * <p>
	 * 返回: void
	 * <p>
	 ...
	 * 描述: 删除
	 * <p>
	 * 参数: @param position 参数: @param iconInfo
	 * <p>
	 * 返回: void
	 * <p>
	 * 异常
	...
	private void deletAnimation(final int position) {
		...
		AnimatorSet animatorSet = createTranslationAnim(position, aimPosit,
				view, animView);
	 ...
	 */
	private AnimatorSet createTranslationAnim(int position, int aimPosit,
			View view, ImageView animView) {
	 ...
	 * <p>
	 * 参数: @param ev 参数: @param scrolly 参数: @return
	 * <p>
	 * 返回: boolean
	 * <p>
	 * 异常
